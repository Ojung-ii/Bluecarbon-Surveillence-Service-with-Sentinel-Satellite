...
def calculate_diff_bg(image,aoi):
        date = ee.Date(image.get('system:time_start')).format('YYYY-MM-dd')
...
def calculateRVI(aoi,start_date,end_date,calculate_rvi):
    ...
    # 시계열 RVI 계산
    time_series_rvi = sentinel1.map(calculate_rvi, aoi)
...
def calculateNDVI(aoi, start_date, end_date,calculate_rvi):
    ...
    time_series_ndvi = sentinel2.map(calculate_rvi)
    # 결과를 서버측 객체로 변환 (Python 클라이언트로 가져오기 위함)
