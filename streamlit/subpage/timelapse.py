...
def app():
    ...
    with col0:
        ...
        if st.toggle("사용설명서"):
            st.write("""
Sentinel-1과 Sentinel-2 위성 데이터를 활용하여 타임랩스를 생성하는 Streamlit 웹 애플리케이션입니다.

1. 데이터셋 및 관심 지역 선택
화면 상단의 드롭다운 메뉴에서 'Sentinel-1' 또는 'Sentinel-2' 데이터셋을 선택합니다.
기존 지정된 관심 지역 중 하나를 선택하거나 '새로운 관심영역 넣기' 옵션을 통해 GeoJSON 파일을 업로드하여 새로운 지역을 정의합니다.
2. 분석 기간 및 주기 설정
시작 날짜와 종료 날짜를 선택하여 분석할 기간을 지정합니다.
주기(일, 월, 분기, 연)를 선택하여 타임랩스의 시간 간격을 설정합니다.
3. 타임랩스 생성
'타임랩스 생성' 버튼을 클릭하여 선택한 지역과 기간에 대한 타임랩스 생성을 시작합니다.
타임랩스 생성 과정 중에는 로딩이 표시되며, 이는 작업이 진행 중임을 나타냅니다.
4. 결과 확인
타임랩스 생성이 완료되면, 생성된 타임랩스 GIF 이미지가 화면에 표시됩니다. 이미지 아래에는 해당 데이터셋의 이름이 캡션으로 표시됩니다.
주의사항
인터넷 연결 상태에 따라 타임랩스 생성 시간이 달라질 수 있습니다.
모든 데이터와 타임랩스는 Google Earth Engine을 통해 제공되는 최신 위성 이미지에 기반합니다.
GeoJSON 파일은 정확한 지리적 경계를 나타내야 하며, 파일 형식이 올바르지 않을 경우 타임랩스 생성이 제대로 수행되지 않을 수 있습니다.
이 사용설명서를 따라 타임랩스 생성기를 사용하면, Sentinel-1과 Sentinel-2 위성 데이터를 활용하여 지정된 기간과 지역에 대한 동적인 시각적 변화를 관찰할 수 있습니다.
                     """)
