        ...
        if on:
            st.write("""
                    식생지수 시계열 경향성 분석을 위한 Streamlit 웹 애플리케이션 입니다.
                        ...
                        1. 관심 지역 설정
                        화면에 표시된 지도에서 관심 있는 지역을 선택합니다. 선택 방법은 두 가지입니다:
                        기존 지역 선택: 드롭다운 목록에서 이미 정의된 관심 지역 중 하나를 선택합니다.
                        새로운 지역 추가: '새로운 관심영역 넣기' 옵션을 선택하고, GeoJSON 형식의 파일을 업로드하여 새로운 지역을 정의합니다.
                        2. 분석 기간 설정
                        시작 날짜와 종료 날짜를 선택합니다. 이 날짜들은 분석할 시계열 데이터의 범위를 결정합니다.
                        3. 분석 실행
                        '분석 실행' 버튼을 클릭하여 선택한 지역과 기간에 대한 식생지수 분석을 시작합니다.
                        4. 식생지수 선택 및 분석 결과 확인
                        각 식생지수(RVI, NDVI, WAVI, DIFF_BG, WEVI, WTDVI)에 대한 탭을 클릭하여 해당 지수의 분석 결과를 확인합니다.
                        각 탭에서는 다음과 같은 정보를 확인할 수 있습니다:
                        ...
                        5. 추가 기능
                        지도 위의 관심 지역에 대한 세부 정보를 확인하거나, 다른 관심 지역을 선택하여 분석을 다시 실행할 수 있습니다.
                    """)
